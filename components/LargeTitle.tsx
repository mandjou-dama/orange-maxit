...
export const LargeTitle: FC<Props> = ({
  ...
  const rTitleOpacityStyle = useAnimatedStyle(() => {
    if (headerBaselineY.value <= 0)
      return { opacity: 0, transform: [{ translateY: 5 }] };
    ...
    return {
      ...
      transform: [
        { translateY: withTiming(offsetY.value > scrollDistance ? 0 : 5) },
      ],
    };
  });
      ...
      headerTitle: (props: HeaderTitleProps) => {
        return (
          ...
          <Animated.View style={rTitleOpacityStyle}>
            <HeaderTitleComponent tintColor="#fff" {...props}>
              {title}
